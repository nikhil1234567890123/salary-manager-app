 ...
 */
export function calculateSavings(monthlySalary: number, savingsRate: number): number {
    return Math.round(monthlySalary * savingsRate);
}

/**
 * Calculate total expenses from expense list
 */
...
export function calculateDailySpending(
    ...
    return Math.round(spendableMoney / remainingDays);
}
